...
public class ReporterLib {
	...
	void generateFooterDetails(PrintWriter m_out) throws UnknownHostException {
		...
		m_out.println("<Table><Hr><I>Test Automation Team</I>");
	}
	...
	// startHtml: Function to start html report with CSS styling and HTML header
	// details
	...
	// generateSuiteSummaryReport - Function to generate suite summary containing
	// key metrics
	...
	// generateMethodSummaryReport - Function to generate details of each method -
	// pass & fail
	...
	public String ArchiveFile(String outdir, String strResultFile) {
		...
		// File source = new File(workingDir + "\\target\\surefire-reports\\" +
		// strResultFile);
...
// ================================================================================================================================
	...
	private void startResultSummaryTable(String style, PrintWriter m_out) {
		...
		m_out.println(
				"<tr bgcolor=\"#D6EAF8\"><th>Module</th><th>Test Script</th><th style=\"width:100px\">Scenario</th>"
...
//==============================================================================================================================
	...
	/*
	 * public void sendMail(String strNewResultFile, String strToEmail) throws
	 * IOException { String strSMTP_Host = "smtp.gmail.com"; String strSMTP_Port =
	 * "465"; final String username = "Chautomailer@chtsinc.com";// final String
	 * password = "Test@12345$";
	 * 
	 * SimpleDateFormat ft = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); long Time
	 * = System.currentTimeMillis(); final String SSL_FACTORY =
	 ...
	 * props.put("mail.smtp.auth", "true"); props.put("mail.store.protocol",
	 * "pop3"); props.put("mail.transport.protocol", "smtp"); try { Session session
	 * = Session.getDefaultInstance(props, new Authenticator() { protected
	 * PasswordAuthentication getPasswordAuthentication() { return new
	 * PasswordAuthentication(username, password); } }); // -- Create a new message
	 * -- Message msg = new MimeMessage(session); // -- Set the FROM and TO fields
	 * -- msg.setFrom(new InternetAddress(username));
	 * msg.setRecipients(Message.RecipientType.TO, InternetAddress.parse(strToEmail,
	 * false)); msg.setSubject("DXL Automation Test Report - " + ft.format(Time));
	 * BodyPart messageBodyPart1 = new MimeBodyPart(); messageBodyPart1.setText(
	 * "Dear Recipient,\n\nPlease find attached test report for DXL Automation.\n\nRegards,\nDXL Automation Team"
	 * ); BodyPart messageBodyPart = new MimeBodyPart();
	 * 
	 * // Fill the message Multipart multipart = new MimeMultipart(); File att = new
	 * File(strNewResultFile); ((MimeBodyPart) messageBodyPart).attachFile(att);
	 * multipart.addBodyPart(messageBodyPart);
	 * multipart.addBodyPart(messageBodyPart1); // Send the complete message parts
	 * msg.setContent(multipart); msg.setSentDate(new Date()); Transport.send(msg);
	 * System.out.println("*****Reporter: Email sent to Recipient - " + strToEmail);
	 * } catch (MessagingException e) { System.out.println("Error arising , cause: "
	 * + e); } }
	 */
//==============================================================================================================================
